...
const ScheduleView: React.FC<ScheduleViewProps> = ({ isEditMode }) => {
  ...
  useEffect(() => {
    const unsubscribe = dbService.subscribeField('schedule', (data) => {
      if (data && typeof data === 'object') {
        ...
        dbService.updateField('schedule', initial);
      }
    });
  ...
  const getWeatherDisplay = (condition: string, hour: string, temp: number) => {
    ...
    switch(condition) {
      ...
      case 'snowy': return 'fa-snowflake text-blue-400'; // 調高飽和度
    ...
    <div className="pb-24 px-4 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-x-hidden">
      <div className="pt-4 flex justify-between items-center px-1">
        ...
        <div className="flex items-center gap-3">
           ...
           <button 
             onClick={() => isEditMode && (setTempMetadata({...currentDayData.metadata}), setSearchQuery(''), setShowWeatherModal(true))}
             className={`text-sm font-bold text-ink bg-white/60 px-5 py-2 rounded-full border border-paper shadow-sm text-center min-w-[120px] tracking-tight flex items-center justify-center gap-2 ${isEditMode ? 'hover:bg-white active:scale-95 transition-all cursor-pointer' : ''}`}
           >
             {currentDayData?.metadata?.locationName || '未設定'}
             {isEditMode && <i className="fa-solid fa-pen text-[9px] opacity-40"></i>}
           </button>
        </div>
      </div>
      ...
      <div className="space-y-2 overflow-x-hidden px-1">
        ...
        {currentDayData?.metadata?.forecast?.length > 0 ? (
          ...
          <div className="py-20 text-center border-2 border-dashed border-paper rounded-[2.5rem] bg-white/10 opacity-60">
            <p className="text-xs font-bold text-earth-dark italic">點擊目的地設定地點以查看氣象</p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-6 overflow-x-hidden relative px-1">
        ...
        )) : <div className="py-20 text-center text-earth-dark/40 italic text-xs font-bold tracking-widest uppercase">本日尚無計畫</div>}
        {isEditMode && (
          <div className="px-1">
