...
const PlanningView: React.FC<PlanningViewProps> = ({ members }) => {
  ...
  const [currentAuthorId, setCurrentAuthorId] = useState<string>(members[0]?.id || '1');
  ...
  const renderMemberSelect = () => (
    <div className="grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {members.map(member => (
        <NordicCard key={member.id} onClick={() => setSelectedMemberId(member.id)} className="flex flex-col items-center py-8 hover:border-sage">
          <img src={member.avatar} className="w-20 h-20 rounded-full border-4 border-slate mb-3 object-cover shadow-sm" alt={member.name} />
          <span className="font-bold text-sage">{member.name}</span>
          <span className="text-[9px] text-earth-dark font-bold uppercase tracking-widest mt-1">View List</span>
        </NordicCard>
      ))}
    </div>
  );
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-300">
        ...
        {activeTab === 'packing' ? (
              ...
              return (
                <div key={cat.id} className="space-y-3">
                  <div 
                    ...
                    className="flex items-center justify-between p-5 bg-[#DDD3C9]/40 border border-[#D2C2B2]/50 rounded-[2rem] cursor-pointer active:opacity-80 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-base font-bold text-sage tracking-tight">{cat.label}</span>
                      <span className="bg-white/60 text-[10px] font-bold text-earth-dark/70 px-2.5 py-0.5 rounded-full border border-paper">
                  ...
                  {isExpanded && (
                    <div className="space-y-2 px-1 pb-2">
                      {catItems.length > 0 ? catItems.map(item => (
                        <div key={item.id} className="bg-white p-5 rounded-[2rem] border border-paper/30 flex items-center justify-between shadow-sm group/item animate-in fade-in slide-in-from-top-1 duration-300">
                          <div onClick={() => toggleItem(item.id)} className="flex items-center gap-4 flex-grow cursor-pointer">
                            <div className={`w-7 h-7 rounded-xl border-2 flex items-center justify-center transition-all ${item.completed ? 'bg-sage border-sage' : 'border-paper/60'}`}>
                              {item.completed && <i className="fa-solid fa-check text-white text-xs"></i>}
                            </div>
                            <span className={`text-base font-bold transition-all ${item.completed ? 'line-through opacity-40 text-sage' : 'text-sage'}`}>{item.text}</span>
                          </div>
                          <button onClick={() => deleteItem(item.id)} className="text-earth-dark/30 hover:text-stamp transition-colors w-10 h-10 flex items-center justify-center">
                            <i className="fa-solid fa-trash-can text-sm"></i>
                          </button>
                        </div>
                      )) : (
                        <div className="py-6 text-center">
                          <p className="text-xs font-bold text-earth-dark/30 italic uppercase tracking-[0.2em]">尚無項目</p>
                        </div>
                      )}
                    </div>
                  )}
          ...
          <div className="space-y-3">
            {items.length > 0 ? items.map(item => (
              <div key={item.id} className="bg-white p-5 rounded-[2.5rem] border border-paper/30 flex items-center justify-between shadow-sm animate-in fade-in duration-300">
                <div onClick={() => toggleItem(item.id)} className="flex items-center gap-4 flex-grow cursor-pointer">
                  <div className={`w-7 h-7 rounded-xl border-2 flex items-center justify-center transition-all ${item.completed ? 'bg-sage border-sage' : 'border-paper/60'}`}>
                    {item.completed && <i className="fa-solid fa-check text-white text-xs"></i>}
                  </div>
        ...
        <button 
          ...
          className="w-full h-16 border-2 border-dashed border-sage/20 rounded-[2rem] bg-white/40 flex items-center justify-center gap-2 text-sage font-bold active:scale-95 transition-all text-sm shadow-md hover:bg-white hover:border-sage mt-6"
        >
  ...
  return (
    <div className="pb-36 px-4 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-x-hidden">
      ...
      <div className="flex bg-white/60 p-1.5 rounded-[1.75rem] border border-paper/40 shadow-inner">
        {(['todo', 'packing', 'shopping', 'info'] as const).map((t) => (
          <button 
            key={t} 
            onClick={() => { setActiveTab(t); setSelectedMemberId(null); }} 
            className={`flex-1 py-3 rounded-[1.25rem] text-[10px] font-bold transition-all uppercase tracking-widest ${activeTab === t ? 'bg-sage text-white shadow-lg' : 'text-earth-dark/60 hover:bg-white/30'}`}
          >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {todos.length > 0 ? todos.map(item => (
              <div key={item.id} className="bg-white p-5 rounded-[2.5rem] border border-paper/30 flex items-center justify-between shadow-md animate-in fade-in slide-in-from-left-2 duration-300">
                ...
                <button onClick={() => deleteTodo(item.id)} className="w-12 h-12 flex items-center justify-center text-earth-dark/20 hover:text-stamp transition-all">
                  <i className="fa-solid fa-trash-can text-sm"></i>
                </button>
              </div>
              ...
              return (
                <div key={info.id} className="relative group animate-in slide-in-from-bottom-2 fade-in duration-300">
                  <div className="absolute -top-4 -right-2 z-10">
                    <img src={author?.avatar} className="w-12 h-12 rounded-full border-4 border-white shadow-xl object-cover" alt="" />
                  </div>
        ...
        <div className="space-y-6 pb-4">
          <div className="space-y-2">
            ...
            <input 
              ...
              className="w-full p-5 bg-white border-2 border-paper/30 rounded-[1.75rem] font-bold text-sage outline-none shadow-sm" 
            />
          </div>
          <div className="space-y-2">
            ...
            <select 
              ...
              className="w-full p-5 bg-white border-2 border-paper/30 rounded-[1.75rem] font-bold text-sage outline-none shadow-sm appearance-none"
            >
              <option value="ALL">全體成員 (Everyone)</option>
              {members.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
          </div>
          <NordicButton onClick={handleAddTodo} className="w-full py-5 bg-sage text-white font-bold mt-2 shadow-xl border-none uppercase tracking-widest">確定新增任務</NordicButton>
        </div>
      </Modal>
      ...
      <Modal isOpen={showAddItemModal} onClose={() => setShowAddItemModal(false)} title={`新增${activeTab === 'packing' ? '行李' : '採買'}項目`}>
        <div className="space-y-6 pb-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {PACKING_CATS.map(cat => (
                  <button 
                    ...
                    className={`py-4 px-3 rounded-2xl text-[11px] font-bold transition-all border-2 flex items-center gap-3 ${newItem.category === cat.id ? 'bg-sage text-white border-sage shadow-lg' : 'bg-white text-earth-dark/60 border-paper/20'}`}
                  >
          ...
          <NordicButton onClick={handleAddItem} className="w-full py-5 bg-sage text-white font-bold mt-4 shadow-xl border-none uppercase tracking-widest">加入清單</NordicButton>
        </div>
      </Modal>
    </div>
  );
